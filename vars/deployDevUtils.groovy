def call(body) {
    ...
    CUSTOM_ENVIRONMENT_CONFIG = CUSTOM_VARS ? vars_file(CUSTOM_VARS) : null
    CUSTOM_ENVIRONMENT_SECRETS = CUSTOM_VAULT_VARS ? vars_file(CUSTOM_VAULT_VARS) : null
                    ...
                    dir('deploy-repo') {
                        script {
                            withCredentials([file(credentialsId: "${KUBECONFIG_CREDENTIAL}", variable: 'KUBE_CONFIG')]) {
                                sh "helm package helm/dev-utils"
                                sh """
                                    helm --kubeconfig ${KUBE_CONFIG} \
                                        upgrade ${APPLICATION} ep-dev-utils-0.1.0.tgz \
                                        --install \
                                        --atomic \
                                        --cleanup-on-fail \
                                        --reset-values \
