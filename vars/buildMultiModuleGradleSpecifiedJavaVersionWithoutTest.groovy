def call(body) {
    ...
    ECR_BASE_PREFIX = config.ecr_base
        ...
        stages {
            ...
            stage('publish-image') {
                steps {
                    dir('code-repo') {
                        script {
                            sh(script: 'echo Publishing all module images')
                            for (mod in MODULES) {
                                IMAGE_TAG = "${ECR_REPO_NAME}/${mod}:latest"
                                ECR_REPO = "${ECR_BASE_PREFIX}/${ECR_REPO_NAME}/${mod}"
                                ECR_TAG_PREFIX = ""
                                ...
                                IMAGE_ALREADY_EXISTS = sh(
                                    returnStdout: true,
                                    script: """
                                        aws ecr list-images --repository-name ${ECR_REPO_NAME}/${mod} \
