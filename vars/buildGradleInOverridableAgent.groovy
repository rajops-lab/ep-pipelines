def call(body) {
    ...
    AGENT_CLOUD = config.agent_cloud ?: 'kubernetes-jdk11'
    AGENT_TEMPLATE = config.agent_template ?: 'kubernetes-jdk11-slave'
    ...
    POSTGRES_CONNECT_URL = config.postgres_connect_url ?: 'jdbc:postgresql://postgresql:5432/postgres'
    POSTGRES_USERNAME = config.postgres_username ?: 'postgres'
    POSTGRES_PASSWORD = config.postgres_password ?: 'fbHZtDBzX5'
        ...
        stages {
            ...
            stage('build') {
                steps {
                    dir('code-repo') {
                        script {
                            sh './gradlew -g /tmp clean assemble'
                        }
                    }
                }
            }
            stage('build-image') {
                steps {
                    dir('code-repo') {
                        script {
                            sh './gradlew -g /tmp clean dockerBuildImage'
                        }
                    }
                }
            }
